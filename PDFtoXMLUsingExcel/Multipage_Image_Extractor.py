...
# Import PIL for panel image combining
try:
    from PIL import Image
    HAS_PIL = True
except ImportError:
    HAS_PIL = False
    print("Warning: PIL/Pillow not available, panel image combining disabled")
...
# ----------------------------
# Panel Image Combining Functions
# ----------------------------

@dataclass
class PanelImageInfo:
    """Store information about an extracted image for panel combining."""
    element: ET.Element
    id: str
    file: str
    x1: float
...
def should_combine_horizontally(img1: PanelImageInfo, img2: PanelImageInfo, 
                                 y_tolerance: float = 10.0,
                                 max_gap: float = 50.0) -> bool:
    """
    Check if two images should be combined horizontally (side-by-side panels).
    
    Criteria:
    1. Same Y position (within tolerance) - they're on the same row
    2. Similar heights - they're meant to be viewed together
    3. Small horizontal gap between them
    """
    # Check if Y coordinates match (same row)
    y1_match = abs(img1.y1 - img2.y1) <= y_tolerance
    y2_match = abs(img1.y2 - img2.y2) <= y_tolerance
    
    if not (y1_match and y2_match):
        return False
    
    # Check if heights are similar (within 20%)
    height_ratio = min(img1.height, img2.height) / max(img1.height, img2.height) if max(img1.height, img2.height) > 0 else 0
    if height_ratio < 0.8:
        return False
    
    # Check horizontal gap - images must be adjacent, not overlapping
    if img1.x2 <= img2.x1:
        gap = img2.x1 - img1.x2
    elif img2.x2 <= img1.x1:
        gap = img1.x1 - img2.x2
    else:
        return False  # Images overlap horizontally
    
    return gap <= max_gap


def should_combine_vertically(img1: PanelImageInfo, img2: PanelImageInfo,
                               x_tolerance: float = 10.0,
                               max_gap: float = 50.0) -> bool:
    """
    Check if two images should be combined vertically (stacked panels).
    
    Criteria:
    1. Same X position (within tolerance) - they're in the same column
    2. Similar widths - they're meant to be viewed together
    3. Small vertical gap between them
    """
    # Check if X coordinates match (same column)
    x1_match = abs(img1.x1 - img2.x1) <= x_tolerance
    x2_match = abs(img1.x2 - img2.x2) <= x_tolerance
    
    if not (x1_match and x2_match):
        return False
    
    # Check if widths are similar (within 20%)
    width_ratio = min(img1.width, img2.width) / max(img1.width, img2.width) if max(img1.width, img2.width) > 0 else 0
    if width_ratio < 0.8:
        return False
    ...
    if img1.y2 <= img2.y1:
        gap = img2.y1 - img1.y2
    elif img2.y2 <= img1.y1:
        gap = img1.y1 - img2.y2
    else:
        return False  # Images overlap vertically
    
    return gap <= max_gap


def find_panel_image_groups(images: List[PanelImageInfo], 
                            y_tolerance: float = 10.0,
                            x_tolerance: float = 10.0,
                            max_gap: float = 50.0) -> List[List[PanelImageInfo]]:
    """
    Group images that should be combined into figures using union-find algorithm.
    """
    if not images:
        return []
    ...
    # Check all pairs of images
    for i in range(n):
        for j in range(i + 1, n):
            if should_combine_horizontally(images[i], images[j], y_tolerance, max_gap):
                union(i, j)
            elif should_combine_vertically(images[i], images[j], x_tolerance, max_gap):
                union(i, j)
    
    # Group images by their root parent
    groups: Dict[int, List[PanelImageInfo]] = {}
...
def combine_images_horizontal(image_paths: List[str], output_path: str) -> bool:
...
def combine_images_vertical(image_paths: List[str], output_path: str) -> bool:
...
def determine_combination_direction(group: List[PanelImageInfo]) -> str:
    """Determine whether a group should be combined horizontally or vertically."""
    if len(group) < 2:
        return 'none'
    
    y_variance = max(img.y1 for img in group) - min(img.y1 for img in group)
    x_variance = max(img.x1 for img in group) - min(img.x1 for img in group)
    
    if y_variance < 20 and x_variance > 50:
        return 'horizontal'
    if x_variance < 20 and y_variance > 50:
        return 'vertical'
    
    return 'horizontal'  # Default


def combine_panel_images_in_xml(
    xml_path: str,
    media_dir: str,
    y_tolerance: float = 10.0,
    x_tolerance: float = 10.0,
    ...
    """
    Post-process the media XML to combine panel images that belong to the same figure.
    
    Detection Logic:
    - Images with matching Y coordinates (same row) → combine horizontally
    - Images with matching X coordinates (same column) → combine vertically
    - Small gap between images confirms they're adjacent panels
    
    The combined image takes the ORIGINAL filename of the first image,
    so existing references in MD/Word/XML files continue to work.
    Original panel images are renamed with _panel1, _panel2 suffixes.
    
    Args:
        xml_path: Path to media.xml file
        media_dir: Directory containing the media files
        y_tolerance: Y coordinate tolerance for horizontal grouping (default: 10.0)
        x_tolerance: X coordinate tolerance for vertical grouping (default: 10.0)
        max_gap: Maximum gap between images to consider adjacent (default: 50.0)
        keep_originals: If True, rename originals with _panel suffix; if False, delete them
    
    Returns:
        Statistics dictionary with combining results
    """
    if not HAS_PIL:
        print("  Panel combining skipped: PIL/Pillow not available")
        return {'groups_found': 0, 'images_combined': 0}
    
    # Parse XML
    tree = ET.parse(xml_path)
    root = tree.getroot()
    
    # Extract image information grouped by page
    images_by_page: Dict[int, List[PanelImageInfo]] = {}
    
    for page_el in root.findall('.//page'):
        page_index = int(page_el.get('index', 0))
        ...
        for media_el in page_el.findall('media'):
            ...
            try:
                img_info = PanelImageInfo(
                    element=media_el,
                    id=media_el.get('id', ''),
                    file=media_el.get('file', ''),
                    x1=float(media_el.get('x1', 0)),
                    y1=float(media_el.get('y1', 0)),
                    x2=float(media_el.get('x2', 0)),
                    y2=float(media_el.get('y2', 0)),
                    page_index=page_index,
                )
                images_by_page[page_index].append(img_info)
    ...
    for page_el in root.findall('.//page'):
        ...
        # Find groups of images to combine
        groups = find_panel_image_groups(images, y_tolerance, x_tolerance, max_gap)
        
        for group in groups:
            if len(group) < 2:
                continue
            
            total_stats['groups_found'] += 1
            direction = determine_combination_direction(group)
            
            # Sort images by position
            if direction == 'horizontal':
                sorted_group = sorted(group, key=lambda img: img.x1)
            else:
                sorted_group = sorted(group, key=lambda img: img.y1)
            
            print(f"    Page {page_index}: Combining {len(group)} panels {direction}ly:")
            for img in sorted_group:
                print(f"      - {img.file}")
            
            # Get file paths
            image_paths = [os.path.join(media_dir, img.file) for img in sorted_group]
            ...
            # The combined image will use the FIRST image's original filename
            first_img = sorted_group[0]
            combined_filename = first_img.file
            combined_path = os.path.join(media_dir, combined_filename)
            
            # Rename all original files to _panel1, _panel2, etc.
            renamed_paths = []
            for i, img in enumerate(sorted_group, start=1):
                original_path = os.path.join(media_dir, img.file)
                base, ext = os.path.splitext(img.file)
                panel_filename = f"{base}_panel{i}{ext}"
                panel_path = os.path.join(media_dir, panel_filename)
                
                os.rename(original_path, panel_path)
                renamed_paths.append(panel_path)
                total_stats['renamed_files'].append((img.file, panel_filename))
            
            # Combine the renamed panel images
            if direction == 'horizontal':
                success = combine_images_horizontal(renamed_paths, combined_path)
            else:
                success = combine_images_vertical(renamed_paths, combined_path)
            
            if not success:
                # Restore original filenames on failure
                for i, img in enumerate(sorted_group, start=1):
                    base, ext = os.path.splitext(img.file)
                    panel_filename = f"{base}_panel{i}{ext}"
                    panel_path = os.path.join(media_dir, panel_filename)
                    ...
                    if os.path.exists(panel_path):
                        os.rename(panel_path, original_path)
                continue
            
            print(f"      → Combined as: {combined_filename}")
            total_stats['images_combined'] += len(group)
            total_stats['combined_files'].append(combined_filename)
            
            # Delete panel files if requested
            if not keep_originals:
                for panel_path in renamed_paths:
                    if os.path.exists(panel_path):
                        os.remove(panel_path)
            
            # Update XML: modify first element, remove others
            combined_x1 = min(img.x1 for img in sorted_group)
            combined_y1 = min(img.y1 for img in sorted_group)
            combined_x2 = max(img.x2 for img in sorted_group)
            combined_y2 = max(img.y2 for img in sorted_group)
            
            # Update first element
            first_el = sorted_group[0].element
            first_el.set('x1', str(combined_x1))
            first_el.set('y1', str(combined_y1))
            first_el.set('x2', str(combined_x2))
            first_el.set('y2', str(combined_y2))
            panel_files = [f"{os.path.splitext(img.file)[0]}_panel{i}{os.path.splitext(img.file)[1]}" 
                           for i, img in enumerate(sorted_group, start=1)]
            first_el.set('combined_from', ','.join(panel_files))
            
            # Remove other elements from XML
            for img in sorted_group[1:]:
                page_el.remove(img.element)
    
    # Save updated XML if changes were made
    if total_stats['groups_found'] > 0:
...
def is_cover_page(
    ...
    """
    Detect if a page is likely a cover page (front or back cover).

    Cover pages typically:
    - Are the first or last page of the document
    - Have minimal text content
    - May be dominated by images or graphics

    Args:
        page: PyMuPDF page object
        page_no: Page number (1-indexed)
        total_pages: Total number of pages in the document
        min_text_chars: Pages with fewer than this many characters are considered potential covers
        max_image_coverage: If images cover more than this ratio of page, likely a cover

    Returns:
        Tuple of (is_cover: bool, reason: str)
    """
    # Only check first and last pages
    if page_no != 1 and page_no != total_pages:
        return False, ""

    page_type = "Front cover" if page_no == 1 else "Back cover"
    ...
    # Cover detection criteria:
    # 1. First or last page with minimal text
    # 2. OR first/last page with significant image coverage
    reasons = []

    if text_len < min_text_chars:
        reasons.append(f"minimal text ({text_len} chars)")

    if image_coverage > max_image_coverage:
        reasons.append(f"high image coverage ({image_coverage:.0%})")

    # For first/last page, even with some text, if there are large images it's likely a cover
    if page_no == 1 or page_no == total_pages:
        # Check for full-page or near-full-page images
        for img in image_list:
            try:
                img_rects = page.get_image_rects(img[0])
                for rect in img_rects:
                    img_area = rect.width * rect.height
                    if img_area / page_area > 0.6:  # Image covers >60% of page
                        reasons.append("large cover image detected")
                        break
            except:
                pass

    if reasons:
        return True, f"{page_type}: {'; '.join(reasons)}"

    # Default: first and last pages are always treated as potential covers
    # This ensures cover images are never cropped
    if page_no == 1:
        return True, f"{page_type}: first page"
    elif page_no == total_pages:
        return True, f"{page_type}: last page"
...
def render_full_page_as_image(
    ...
    filename = f"p{page_no}_fullpage.png"
...
def extract_raster_images_for_page(
    ...
    for img in images:
        ...
        for rect in rects:
            ...
            img_counter += 1
            filename = f"p{page_no}_img{img_counter}.png"
            out_path = os.path.join(media_dir, filename)
...
def extract_vector_blocks_for_page(
    ...
    for rect in clustered_rects:
        ...
        vec_counter += 1
        filename = f"p{page_no}_vector{vec_counter}.png"
        out_path = os.path.join(media_dir, filename)
...
def add_tables_for_page(
    ...
    for idx, t in enumerate(tables_for_page, start=1):
        ...
        # Calculate ncols as the MAXIMUM across all rows to handle merged cells correctly
        # Different rows may have different cell counts due to merged cells or detection issues
        ncols = max((len(row) for row in cells), default=0) if nrows > 0 else 0
        ...
        for r_idx in range(nrows):
            ...
            if structure_rect or text_validated_rect:
                ...
                row_cells = cells[r_idx]
                row_cell_count = len(row_cells)
                for c_idx_check in range(row_cell_count):  # Use actual row cell count
                    cell_check = row_cells[c_idx_check]
            ...
            current_row_cells = cells[r_idx]
            current_row_cell_count = len(current_row_cells)

            # Always create ncols entries for consistent table structure
            for c_idx in range(ncols):
                if c_idx < current_row_cell_count:
                    # Cell exists in this row
                    cell = current_row_cells[c_idx]
                    ...
                    # Row has fewer cells than ncols - create empty entry for missing cell
                    # This can happen with merged cells or detection issues
                    entry_el = ET.SubElement(
                        row_el,
                        "entry",
                        {
...
def extract_media_and_tables(
    ...
    # Save MultiMedia.xml alongside the media folder (in its parent directory)
    # This keeps the XML and media folder together for consistent path resolution
    if out_dir:
        xml_parent = os.path.dirname(media_dir)
        xml_path = os.path.join(xml_parent, f"{base_name}_MultiMedia.xml")
    else:
    ...
    for page_index in range(num_pages):
        ...
        # Check for cover pages (front/back) - render WITHOUT cropping
        is_cover, cover_reason = is_cover_page(page, page_no, num_pages)
        if is_cover:
            # Mark page as full-page-image cover in XML
            page_el.set("render_mode", "fullpage")
            page_el.set("render_reason", cover_reason)
            page_el.set("is_cover", "true")
            ...
            render_full_page_as_image(
                ...
                dpi=dpi,
                reason=cover_reason,
                crop_margins=False,  # Do NOT crop cover pages - keep entire image
                header_margin_pct=0.0,
                footer_margin_pct=0.0,
                left_margin_pct=0.0,
                right_margin_pct=0.0,
            )

            # Skip normal text/media extraction for cover pages
            continue
    ...
    # Export reference mapper so downstream tools can use the image mappings
    if HAS_REFERENCE_MAPPER:
        try:
            mapper = get_mapper()
            mapper_dir = os.path.dirname(xml_path)
            mapper_path = os.path.join(mapper_dir, f"{base_name}_reference_mapping_phase1.json")
            mapper.export_to_json(Path(mapper_path))
            print(f"  ✓ Reference mapper exported: {mapper_path} ({len(mapper.resources)} resources)")
        except Exception as e:
            print(f"Warning: Could not export reference mapper: {e}")

    # Post-process to combine panel images that belong to the same figure
    if HAS_PIL:
        print(f"\n{'='*60}")
        print("POST-PROCESSING: Combining panel images")
        print(f"{'='*60}")
        try:
            combine_stats = combine_panel_images_in_xml(
                xml_path=xml_path,
                media_dir=media_dir,
                y_tolerance=10.0,
                x_tolerance=10.0,
                max_gap=50.0,
                keep_originals=True,  # Keep original panels as backup
            )
            
            if combine_stats['groups_found'] > 0:
                print(f"\n  Panel Combining Results:")
                print(f"    Image groups found: {combine_stats['groups_found']}")
                print(f"    Images combined: {combine_stats['images_combined']}")
                if combine_stats['combined_files']:
                    print(f"    Combined images (using original filename):")
                    for f in combine_stats['combined_files']:
                        print(f"      - {f}")
                if combine_stats['renamed_files']:
                    print(f"    Original panels renamed (kept as backup):")
                    for old_name, new_name in combine_stats['renamed_files'][:6]:  # Show first 6
                        print(f"      - {old_name} → {new_name}")
                    if len(combine_stats['renamed_files']) > 6:
                        print(f"      ... and {len(combine_stats['renamed_files']) - 6} more")
            else:
                print("  No panel images to combine (no adjacent panels detected)")
            print(f"{'='*60}\n")
        except Exception as e:
            print(f"Warning: Panel image combining failed: {e}")
            print(f"Continuing with uncombined images")
    else:
        print(f"\nNote: Panel image combining skipped (PIL/Pillow not available)")
