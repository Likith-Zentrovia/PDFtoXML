...
class ValidationReportGenerator:
    ...
    def __init__(self):
        """Initialize empty report generator."""
    ...
    def generate_excel_report(self, output_path: Path, book_title: str = "Unknown"):
        ...
        if not OPENPYXL_AVAILABLE:
            logger.error("openpyxl is not available, cannot generate Excel report")
            return
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"File is locked (attempt {attempt + 1}/{max_retries}). "
                                  f"Retrying in {wait_time} seconds...")
                    time.sleep(wait_time)
                else:
                    logger.error(f"Cannot save validation report - file is locked")
...
def generate_validation_report(
    ...
    if generator.has_errors():
        ...
        logger.info(f"No validation errors found for {book_title}")
